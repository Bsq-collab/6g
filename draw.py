...
def add_sphere( points, cx, cy, cz, r, step ):
    ...
    for pnt in pnts:
        add_edge(points,pnt[0],pnt[1],pnt[2],pnt[0]+1,pnt[1]+1,pnt[2]+1)

def generate_sphere( points, cx, cy, cz, r, step ):
    pnts=[]
    for i in range(step+1):
        for j in range(step+1):
            rotation = float(i)/step
            circ = float(j)/step
            x=r*math.cos(circ*math.pi) + cx
            y=r*math.sin(circ*math.pi)*math.cos(rotation*2*math.pi) + cy
            z=r*math.sin(circ*math.pi)*math.sin(rotation*2*math.pi) + cz
...
def generate_torus( points, cx, cy, cz, r0, r1, step ):
    ...
    for i in range(step+1):
        for j in range(step+1):
            rotation = float(i)/step
            circ = float(j)/step
            x = math.cos(rotation*2*math.pi)*(r0*math.cos(circ*2*math.pi)+r1) + cx
